...
filters.realtime = function(block, compiler, attrs){
  var filename = compiler.options.filename;
  ...
  block[0].setAttribute('class', ' + " nodestream nodestream_" + ' + subscribeString);
  block.push(new nodes.Filter('javascript', new nodes.Text('Nodestream.register(\'#{'+ subscribeString +'}\');')));
   
  var compiled = [];
 
  if (attrs.repaint){
    var cc = new NodestreamRealtime(block[0].block, compiler.options)
    compiled['repaint'] = new Function('locals', 'var buf = []; with (locals) {return ' + cc.compile() + '}');
  }
  
  fileBlocks[filename].push(compiled);
 
  return new NodestreamRealtime(block, compiler.options).compile();
  ...
  if (attrs.append){
    ...
    var cc = new NodestreamRealtime(block);
    compiled['append'] = new Function('locals', 'var buf = []; with (locals) {' + cc.compile() + '} return buf.join("")');
  ...
  if (attrs.repaint){
    var cc = new NodestreamRealtime(block[0].block);
    compiled['repaint'] = new Function('locals', 'var buf = []; with (locals) {' + cc.compile() + '} return buf.join("");');
  }
  ...
  if (attrs.append){
    //compiler.visit(bl);
    return new NodestreamRealtime(bl).compile();
  } else {
    return new NodestreamRealtime(block).compile();
...
function NodestreamRealtime(node, options) {
...
NodestreamRealtime.prototype.__proto__ = Compiler.prototype;

NodestreamRealtime.prototype.visitTag = function(node) {
  switch (node.name) {
    default:
