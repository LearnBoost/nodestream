...
filters.realtime = function(block, compiler, attrs){
  ...
  if (attrs.append){
    ...
    var js = new NodestreamRealtime(block, compiler.options).compile();
    compiled['append'] = new Function('locals', ''
                + 'var buf = [];\n'
                + 'with (locals) {' + js + '}'
                + 'return buf.join("");');
  ...
  if (attrs.repaint){
    var js = new NodestreamRealtime(block[0].block, compiler.options).compile();
    compiled['repaint'] = new Function('locals', ''
                + 'var buf = [];\n'
                + 'with (locals) {' + js + '}'
                + 'return buf.join("");');
  }
  ...
  if (attrs.append){
    return new NodestreamRealtime(bl, compiler.options).compile();
  } else {
...
NodestreamRealtime.prototype.visitCode = function(code){
    // Wrap code blocks with {}.
    // we only wrap unbuffered code blocks ATM
    // since they are usually flow control

    // LEAVING IN UNTIL JADE IS PATCHED
